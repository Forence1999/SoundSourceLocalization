...
import random
import tempfile
from lib.utils import plot_curve, plot_hist, otsu_threshold
import shutil
...
def audio_energy_ratio_over_threshold(audio, fs=16000, threshold=None, path=None):
    audio_energy = single_side_fft(normalize(audio)) ** 2
...
def single_side_fft(audio, fs=16000):  # 其实可以直接用   np.abs(rfft(audio)) / fs * 2
    fft_res = fft(audio)  # 快速傅里叶变换
    
    fft_amp = np.abs(fft_res)  # 取复数的绝对值，即复数的模(双边频谱)
    norm_fft_amp = fft_amp  # 归一化处理（双边频谱）
    norm_half_fft_amp = norm_fft_amp[:int(fs / 2)]  # 由于对称性，只取一半区间（单边频谱）
    
    return norm_half_fft_amp


def audio_segmenter(input_path, dest_dir, segment_len=10, overlap_per=0., fs=None, dropout=False, threshold=None):
    '''Segments the audio clips to segment_len in secs'''
    '''
